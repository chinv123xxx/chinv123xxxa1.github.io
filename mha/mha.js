let orders = JSON.parse(localStorage.getItem("orders_vjgroup")) || [];
...
window.onload = () => {
  if (orders.length > 0) {
    ...
    if (!useOld) {
      orders = [];
      localStorage.removeItem("orders_vjgroup");
    }
  }
  render();
};
...
function saveData() {
  localStorage.setItem("orders_vjgroup", JSON.stringify(orders));
}
